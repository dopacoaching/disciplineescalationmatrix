...
export async function PATCH(req: NextRequest, { params }: Ctx): Promise<NextResponse> {
  try {
    ...
    await writeAuditLog({ action: 'batch.update', actorId: user.id, actorUsername: user.username, actorRole: user.role, targetType: 'batch', targetId: batch._id.toString(), targetName: batch.name });
